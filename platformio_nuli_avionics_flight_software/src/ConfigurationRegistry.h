...
DEFINE_CONFIGURATION_VARIABLE(FIRMWARE_VERSION, ConfigurationString<20>, AVIONICS_ARGUMENT_firmwareVersion, strcmp(value.str, AVIONICS_ARGUMENT_firmwareVersion) == 0);
...
DEFINE_CONFIGURATION_VARIABLE(LAUNCH_ANGLE, Quaternion, Quaternion(), true)
...
DEFINE_CONFIGURATION_VARIABLE(LORA_SPREADING_FACTOR, int32_t, 8, value >= 5 && value <= 12)
DEFINE_CONFIGURATION_VARIABLE(RADIO_TRANSMIT_INTERVAL, uint32_t, 1000, value < 60 * Units::S_TO_MS)
DEFINE_CONFIGURATION_VARIABLE(BUZZER_ENABLED, uint32_t, true, value == false || value == true)
DEFINE_CONFIGURATION_VARIABLE(GROUND_STATION_MODE, uint32_t, false, value == false || value == true)
